...
def get_all_products_data(query):
...
class ProductDataAPIView(APIView):
    def get(self, request):
        ...
        if not query:
            return Response({"error": "Параметр 'query' обязателен."}, status=400)

        try:
            df = get_all_products_data(query=query)
...
def get_filter_products_data(query, min_price=0, max_price=1e9, min_rating=0, max_rating=5, min_feedbacks=0, max_feedbacks=1e6):
    import pandas as pd
    import requests
    import math
    from sqlalchemy import create_engine
    ...
    # Сохраняем для отладки/аналитики (опционально)
    engine = create_engine("sqlite:///products.db")
    df.to_sql("products", engine, if_exists="replace", index=False)
